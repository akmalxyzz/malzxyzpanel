...
let { directLink } = await service.uploadFromBinary(buffer, 'malzxyz.png');
            resolve(directLink);
        } catch (error) {
...
async function checkQRISStatus(apikey, username, token) {
    try {
        const apiUrl = `https://api-malzxyz.vercel.app/orderkuota/mutasiqr?apikey=${apikey}&username=${username}&token=${token}`;
...
// Contoh penggunaan:
// checkQRISStatus('free', 'akmalygy', '832209:jyUCpMR241vQI3qtNWYFmzcfblnASak6');
