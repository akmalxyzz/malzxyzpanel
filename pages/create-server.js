...
export async function getServerSideProps(context) {
  ...
  try {
    ...
    if (storeRes.ok) {
      ...
      storeSettings = { storeName: "Panel Malzxyz Store", storeDescription: "Gagal memuat deskripsi default." };
    }
  } catch (error) {
    console.error("Error fetching store settings:", error);
    storeSettings = { storeName: "Panel Malzxyz Store", storeDescription: "Error koneksi saat memuat pengaturan." };
  }
...
export default function CreateServerPage({ settings, serversData }) {
  ...
  return (
    <>
      <Head>
        <title>Buat Server Baru - {settings?.storeName || 'Panel Malzxyz Store'}</title>
